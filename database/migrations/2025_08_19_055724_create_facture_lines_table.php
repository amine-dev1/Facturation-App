...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('facture_lines', function (Blueprint $table) {
            ...
            $table->foreignId('facture_id')->constrained('factures')->onDelete('cascade'); // Clé étrangère avec suppression en cascade
            ...
            $table->decimal('taux_tva', 5, 2); // Taux de TVA applicable
            $table->decimal('montant_ht', 10, 2); // Montant total hors taxes
            $table->decimal('montant_tva', 10, 2); // Montant de la TVA calculée
            $table->decimal('montant_ttc', 10, 2); // Montant total TTC (TVA incluse)
            $table->timestamps(); // creation de la collone created_at et updated_at
        });
        
        // Note: Possibilité d'ajouter des champs supplémentaires selon les besoins futurs
    }
